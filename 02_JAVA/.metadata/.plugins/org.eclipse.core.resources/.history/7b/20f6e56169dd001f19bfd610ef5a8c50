...
//		Member m = new Member(userid, userpwd, username, gender, Integer.parseInt(age), email, phone);
	...
	public void movieReservation(String userid, String seatrow, int seatnumber) {// 회원 아이디, 티켓 수량, 좌석번호 일단은 티켓 수량은 빼고
		Member m = new Member();
		Seat seat = new Seat();
		
		int result = is.movieReservation(userid, seatrow, seatnumber);
