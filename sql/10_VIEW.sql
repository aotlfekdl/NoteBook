/*
    ...
    (자주사용하는 SELECT문을 저장해두면 긴 SELECT문을 매번 다시 기술할 필요 없이 사용할 수 있다.)
    가상테이블 -> 실제데이터가 담겨있는 것은 아니다.(논리테이블)
*/
...
--실제 실행되는것은 서브쿼리(인라인뷰)로 실행된다고 볼 수 있다.
...
--CREATE OR REPLACE 
--VIEW 가 없을 때는 생성, 이미 존재한다면 수정할 수 있다.
...
AS (SELECT EMP_ID, EMP_NAME, JOB_NAME,
        DECODE(SUBSTR(EMP_NO,8,1), '1', '남자', '2', '여자') AS "GENDER",
...
--==============================================================================
--생성된 뷰을 통해서 DML(INSERT, UPDATE, DELETE)사용가능하다.
--뷰을 통해서 조작하게되면 실제데이터가 담겨있는 테이블에 반영된다.

CREATE OR REPLACE VIEW VW_JOB
AS (SELECT JOB_CODE, JOB_NAME
    FROM JOB);

SELECT * FROM VW_JOB; -- 논리테이블(실제데이터가 담겨있지 않다.)

SELECT JOB_CODE, JOB_NAME -- 이것은 실제 테이블
    FROM JOB;
...
/*
    ...
    WITH READ ONLY : 뷰에 대해서만 조회하도록
*/

--FORCE | NOFORCE
CREATE OR REPLACE NOFORCE VIEW VW_EMP
AS (SELECT TCODE, TNAME, TONTENT FROM TT);
