--1. 학생이름과 주소지를 표시하시오. 단, 출력 헤더는 "학생 이름", "주소지"로 하고, 정렬은
--이름으로 오름차순으로 표시하도록 한다.
...
--학점이 높ㅇ느 학생부터 표시하고, 학점이 같으면 학번이 낮은 학생부터 표시하는 구문을 작성
...
--9. 8번의 결과 중 'DLSANSTKGHL' 계열에 속한 과목의 교수 이름을 찾으려고 한다. 이에
...
--10. '음악학과' 학생들의 평점을 구하려고한다. 음악학과 학생들의 "학번", "학생 이름",
--"전체 평점"을 출력하는 SQL문장을 작성하시오.(단, 평점은 소수점 1자리까지만 반올림하여 표시한다.)
...
--11. 학번이 'A313047'인 학생이 학교에 나오고 있지 않다. 지도교수에게 내용을 전달하기 위한
...
--13. 예체능 계열 과목 중 과목 담당교수를 한명도 배정받지 못한 과목을 찾아 그 과목이름과
--학과 이름을 출력하는 SQL문장을 작성하시오

--========실패
SELECT DISTINCT DEPARTMENT_NO,CLASS_NAME, DEPARTMENT_NAME
FROM TB_CLASS
...
WHERE CLASS_NO IN (SELECT CLASS_NO
                             FROM TB_CLASS
                             WHERE NOT CLASS_NO IN (SELECT CLASS_NO
                              FROM TB_CLASS_PROFESSOR))
...
;
CATEGORY = '예체능' AND COACH_PROFESSOR_NO IS NULL;
...
--학생이름과 지도교수 이름을 찾고 만일 지도 교수가 없는 학생일 경우 "지도교수 미지정"
--으로 표시하도록 하는 SQL문을 작성하시오. 단, 출력헤더는 "학생이름", "지도교수"로
--표시하며 고학번 학생이 먼저 표시되도록한다.

SELECT STUDENT_NAME, NVL(COACH_PROFESSOR_NO,'지도교수 미지정')
FROM TB_STUDENT
...
SELECT DEPARTMENT_NO, STUDENT_NAME, PROFESSOR_NAME
FROM TB_STUDENT
JOIN TB_PROFESSOR USING(DEPARTMENT_NO)
WHERE DEPARTMENT_NAME IN ()

;
--15. 휴학생이 아닌 학생 중 평점이 4.0이상인 학생을 찾아 그 학생의 학번 이름,
--학과이름, 평점을 출력하는 SQL문을 작성하시오.
SELECT STUDENT_NO, STUDENT_NAME, DEPARTMENT_NAME, AVG(POINT)
FROM TB_STUDENT
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
JOIN TB_GRADE USING(STUDENT_NO)
WHERE ABSENCE_YN = 'N' AND AVG(POINT) >= (SELECT AVG(POINT)
                                            FROM TB_GRADE
                                            GROUP BY STUDENT_NO
                                            HAVING AVG(POINT));
