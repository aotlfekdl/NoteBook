...
/*
    *NOT NULL
    해당컬럼에 반드시 값이 존재해야만 하는경우(즉, 절대 NULL이 들어오면 안되는 경우)
    삽입/ 수정시 NULL값을 허용하지 않도록 제한
    
    제약조건을 부여하는 방식은 크게 2가지가 있다.(컬럽레벨방식, 테이블 레벨방식)
    NOT NULL제약조건은 무조건 컬럼레벨 방식만 가능    
*/
...
INSERT INTO MEMBER
VALUES(3,'USER2', 'PASS3', '홍길TKA', NULL, NULL, NULL, NULL);
--아이디가 중복되었음에도 불구하고 잘 추가가 된다.
...
/*
    *UNIQUE 제약조건
    ...
    삽입/수정시 기존에 있는ㄷ ㅔ이터 값중 중복값이 있을 경우 오류를 발생시킨다.
*/
...
INSERT INTO MEMBER
VALUES(3,'USER2', 'PASS3', '홍길삼', NULL, NULL, NULL, NULL);
--UNIQUE 제약조건에 위배되었으므로 INNSERT 실패
--> 오류구분을 제약조건명으로 알려준다.
...
/*
    *CHECK(조건식)
    해당컬럼에 들어올 수 있는 값에 대한 조건을 제시해 줄 수 있다.
...
--GENDER 컬럼에는 CHECK 제약조건을 만족하는 값을 넣어야 한다.
--단, NULL은 값이 없다는 뜻이기 때문에 가능하다.

INSERT INTO MEMBER_CHECK
VALUES(2,'USER2', 'PASS2', '홍길순', '여', '010-1111-2222', 'AAA@NAVER.COM', '24/12/23');

--==============================================================================
/*
    PRIMARY KEY(기본키) 제약조건
    테이블에서 각 행(ROW)를 식별하기 위해 사용될 컬럼에 부여한느 제약조건(식별자 역할)
    
    EX) 회원번호, 학번, 군번, 부서코드, 직급코드, 주민등록번호, 택배운송장번호, 예약번호
    FRIMARY KEY -> NOT NULL + UNIQUE
...
--기본키에 중복값을 넣으려고 할 때 (UNIQUE제약조건에 반)

INSERT INTO MEMBER_PRI
VALUES(NULL,'USER2', 'PASS2', '홍길순', NULL, '010-1111-2222', 'AAA@NAVER.COM', '24/12/23');
--기본키에 NULL을 담으려고 할 때 (NOT NULL제약조건 위배)

--복합키 : 두개의 컬럼을 동시에 하나의 PORMARI KEY 로 지정한 것
...
INSERT INTO MEMBER_PRI2
VALUES(2,'USER2', 'PASS1', '홍길동', '남', '010-1111-2222', 'AAA@NAVER.COM', '24/12/23');
...
--복합키 사용 예(어떤 회원이 어던 상품을 찜햇는지에 대한 데이터를 보관하는 데이블
...
--복합키 사용 예(어떤 회원이 어던 상품을 찜햇는지에 대한 데이터를 보관하는 데이블
INSERT INTO TB_LIKE VALUES(1,'자전거A', SYSDATE); --홍길순이 자전거 a좋아요 누름
INSERT INTO TB_LIKE VALUES(1,'자전거B', SYSDATB); --홍길순이 자전거 a좋아요 누름
INSERT INTO TB_LIKE VALUES(1,'자전거A', SYSDATE); --홍길순이 자전거 a좋아요 누름
INSERT INTO TB_LIKE VALUES(2,'자전거B', SYSDATE); --홍길순이 자전거 a좋아요 누름


--==============================================================================
--회원등급에 대한 데이트를 보관하는 테이블
...
INSERT INTO MEM
VALUES(1,'USER1', 'PASS1', '홍길동',NULL, '010-1111-2222', 'AAA@NAVER.COM', '24/12/23', NULL);

INSERT INTO MEM
VALUES(2,'USER2', 'PASS2', '홍길순',NULL, '010-1111-2222', 'AAA@NAVER.COM', '24/12/23', 10);
...
INSERT INTO MEM
VALUES(4,'USER4', 'PASS4', '홍길사',NULL, '010-1111-2222', 'AAA@NAVER.COM', '24/12/23', 20);

--유효하지 않은 회원등급 번호가 정상적으로 INSERT된다.
INSERT INTO MEM
VALUES(5,'USER5', 'PASS5', '홍길오',NULL, '010-1111-2222', 'AAA@NAVER.COM', '24/12/23', 50);
/*
    ...
    -> 테이블 레벨 방식 
    FORIGN KEY (컬럼명) REFERENCES 참조할 테이블 명[참조할 컬럼명]
...
--MEM_GRADE(부모테이블) -1--------< ㅡMEM(자식테이블)
--1:N 관계가 성립 1쪽이 부모테이블 N이 자식테이블
...
DELETE FROM MEM_GRADE 
WHERE GRADE_CODE = 40;
--MEM테이블에서 40이라는 값을 사용하고 있지 않기 때문에 삭제가 된다.

--자식테이블에서 이미 사용되고 있는 값이 있을 경우
--부모테이블로부터 무조건 삭제가 안되는 "삭제제한"옵션이 걸려있다.
--===============================================================================
/*
    자식테이블 생성 싯 외래키 제약조건 부여할 때 삭제옵션 지정 기능
    *삭제 옵션: 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 할 것인가?
...
INSERT INTO MEM
VALUES(1,'USER1', 'PASS1', '홍길동',NULL, NULL, NULL, NULL, NULL);

INSERT INTO MEM
VALUES(2,'USER2', 'PASS2', '홍길순',NULL,  NULL, NULL, NULL, 10);
...
--유효하지 않은 회원등급 번호가 정상적으로 INSERT된다.
INSERT INTO MEM
VALUES(5,'USER5', 'PASS5', '홍길오',NULL, NULL, NULL, NULL, 50);
...
/*
    ...
    컬럼을 선정하지 않고 INSERT시에 NULL아닌 기본값을 INSERT하고자 한다.
    미리 값을 세팅해 둘 수 있다.
...
-->단 해당컬럼에 DEFAULT 값이 부여되어 있을 경우 NULL이 아닌 DEFAULT값이 들어감
